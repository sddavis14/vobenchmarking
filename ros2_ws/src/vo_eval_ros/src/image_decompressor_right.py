...
class ImageDecompressor(Node):
    def __init__(self):
        super().__init__('image_decompressor_right')
        self.right_sub = self.create_subscription(
            CompressedImage,
            'camera_right/image_rect/compressed',
            self.right_cam_sub,
            10)
        self.right_sub  # prevent unused variable warning

        self.right_pub = self.create_publisher(RosImage, 'camera_right/image_rect', 10)

    def right_cam_sub(self, msg):
        print('publish right')
        self.right_pub.publish(decompress(msg))
